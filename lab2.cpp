...
class Node{
	public:
	    //this linkedlist consist of two parts
	    //one is the data stored and the second
	    //is the adress
		int data;
...
class Linkedlist{
	public:
		...
		Node * head;
		//a linklist has several fns like inserting a node at the end of the linkedlist
		Node * tail;
		Linkedlist(){
			head=NULL;
			tail=NULL;
		}
		//we are going to create a heap with the data stored
		//which will save its address
		void insrt(int no){

			Node * temp=new Node;
			temp->data=no;
            //linking inthe linkedlist
        ...
        //inserting unit at a specified location of alinkedlist
		void insrtat(int pos,int no){
			...
			//now wew will reach at desired position
			Node * temp=new Node;
			temp->data=no;
		...
		//we are creating afunction to delete the last node
		void deleteit(){
			Node * temp=tail;
		...
		void countitems(){
		    //we are using while looping for counting
			Node * current =head;
			int i=0;
			//while looping
			while(current!=NULL){
			...
			//showing the counted number to the user
